...
//    TJQ_Control(T4,0,1);//右后
    TJQ_Control(T6,sudu5,1);//左中
    TJQ_Control(T7,sudu6,0);//右中
}
...
{ float sudu5=0;
  ...
  //delay_ms(100);          //延时100ms
	while(1)
	{
		...
		ycha=yaw-yaw1;
		if(ycha>=75)
		{// 右转大于90度，即转过第一个拐角，慢速下潜
		    TJQ_Control(T6,25,1);//左中
				TJQ_Control(T7,25,0);//右中
		 }
		 else if((ycha>=165)&&(Sensor_HP20X_GetDepth()<200))
		{// 右转大于165度，即转过第二个拐角
				if(Sensor_HP20X_GetDepth()<200)
				{//深度小于200mm，下潜
						TJQ_Control(T6,60,1);//左中
						TJQ_Control(T7,60,0);//右中
				}
				else if(Sensor_HP20X_GetDepth()<250)
				{//悬浮
						TJQ_Control(T6,45,1);//左中
						TJQ_Control(T7,45,0);//右中
				}
				else
				{
						TJQ_Control(T6,0,1);//左中
						TJQ_Control(T7,0,0);//右中
				}
