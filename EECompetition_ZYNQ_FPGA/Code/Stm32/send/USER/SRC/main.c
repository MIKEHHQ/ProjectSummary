 ...
 // RU_RD=0;
	while(1)
	{
		...
		if(keyc == 'C' )
		{
			...
			nums[4]=0;
			nums[5]=0;
			TM1637_display(nums[0],nums[1],nums[2],nums[3],0);
		...
		if(keyc<10 && n<4)
		{
			...
			n++;
			cnt++;
			TM1637_display(nums[2],nums[3],nums[4],nums[5],0);
			
		}
		if(cnt>=4 && keyc == 'A')
			{
			  //n=0;	
				Write_frequence(0,12000);
...
//				if(send)
//				{
//						TIM_Cmd(TIM3, DISABLE);  //失能能TIMx					 
//					Delay_ms(800);
//				for(j=0;j<64;j+=4)
//				{
//					TM1637_display(code[j],code[j+1],code[j+2],code[j+3],1);
//					Delay_ms(800);
//					//USART_SendData(USART1, code[j]);//向串口1发送数据
//					//while(USART_GetFlagStatus(USART1,USART_FLAG_TC)!=SET);//等待发送结束
//				}
//				send=0;
//				TIM_Cmd(TIM3, ENABLE);  //失能能TIMx				
//				}

			//}
//		TM1637_display(keyc,2,3,4,1);
		//Delay_us(100);	
	}
	if (Type==1)    //调频输出
	{
